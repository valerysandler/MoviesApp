...
export const removeFromFavorites = async (movieId: number) => {
  const res = await apiFetch(`/api/movies/favorites/${movieId}`, {
    method: 'DELETE',
  });
...
export const updateMovie = async (movie: Movie): Promise<Movie> => {
  const res = await apiFetch(`/api/movies/${movie.id}`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
    },
...
export const checkFavoriteStatus = async (movieId: number, userId: string): Promise<boolean> => {
  const res = await apiFetch(`/api/movies/favorites/check?movieId=${movieId}&userId=${userId}`);
...
export const toggleFavorite = async (movieId: number, userId: string): Promise<boolean> => {
  const res = await apiFetch('/api/movies/favorites/toggle', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
